...
begin
  ...
  try
    FCache.ListPackageVersions('hl7.fhir.tools.validator', ts);
    if ts.Count > 0 then
      FJarPath := path([FCache.Folder, 'hl7.fhir.tools.validator-'+ts[ts.Count-1], 'bin', 'org.hl7.fhir.validator.jar']);
  finally
    ts.Free;
  end;
end;
