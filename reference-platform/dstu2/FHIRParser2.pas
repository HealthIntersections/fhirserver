...
begin
  case format of
    ffXml : result := FHIRParserXml2.TFHIRXmlComposer.Create(worker, style, lang);
    ffJson : result := FHIRParserJson2.TFHIRJsonComposer.Create(worker, style, lang);
...
begin
  case format of
    ffXml: result := FHIRParserXml2.TFHIRXmlParser.Create(worker, lang);
    ffJson: result := FHIRParserJson2.TFHIRJsonParser.Create(worker, lang);
  else
    raise Exception.Create('Unspecified/unsupported format');
  end;
end;
