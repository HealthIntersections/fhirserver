...
public class DefinitionsLoader5 {
  ...
  public Definitions loadDefinitions(String ver) throws Exception {
    ...
    for (StructureDefinition sd : sdl) {
      def.setGenDate(new VersionConvertor_10_50(null).convertDateTime(sd.getDateElement()));
    ...
    for (OperationDefinition od : odl) {
      def.getOperations().add(VersionConvertor_30_50.convertOperationDefinition(od));
    }
  ...
  private void processSearchParam(Definitions def, SearchParameter sp) throws Exception {
    for (CodeType ct : sp.getBase()) {
      SearchParameterDefn spd = new SearchParameterDefn().loadR5(sp.getCode(), pickDescription(sp.getDescription(), ct.asStringValue()), new VersionConvertor_10_50(null).convertSearchParamType(sp.getType()), 
          sp.getTarget(), new VersionConvertor_10_50(null).convertXPathUsageType(sp.getXpathUsage()), sp.getExpression());
  ...
  private void processResource(Definitions def, StructureDefinition sd, Map<String, ValueSet> vsmap) throws Exception {
    ...
    if (sd.hasBaseDefinition()) {
      if (ToolingExtensions.hasExtension(sd.getBaseDefinitionElement(), ToolingExtensions.EXT_CODE_GENERATION_PARENT)) { 
        ...
        type.getTypes().add(new TypeRef(sd.getBaseDefinition().substring(50)));
    }
  ...
  private static DefinedCode makePrimitive(StructureDefinition sd) {
    ...
    if (!sd.getBaseDefinition().equals("http://hl7.org/fhir/StructureDefinition/Element")) {
      ...
      dsp.setBase(sd.getBaseDefinition().substring(50));
      return dsp;
    } else {
