...
{$ENDIF}
  ...
  IDirect3DSurface9 = interface(IDirect3DResource9)
    ...
    function GetContainer(const riid: TGUID; out ppContainer: Pointer): HResult; stdcall;   // ct3333 ????
...
begin
  ...
  begin
    ...
    if Direct3D9Loaded then
    begin
      _Direct3DCreate9:= TDirect3DCreate9(GetProcAddress(Direct3D9Lib, ProcName));
      Result:= Assigned(_Direct3DCreate9);
      if not Result then UnLoadDirect3D9;
      {$IFDEF DIRECT3D_VERSION_9_VISTA}
      Direct3DCreate9Ex:= TDirect3DCreate9Ex(GetProcAddress(Direct3D9Lib, ProcNameEx));
      {$ENDIF}
    end;
  end;
end;
