...
procedure TScintEdit.StyleNeeded(const EndPos: Integer);
  ...
  begin
    ...
    { We don't pass line endings to the styler, because when the style of a
      ...
      immediately repaints all subsequent lines.}

    FStyler.FFirstLine := FirstLine;
    FStyler.FLastLine := LastLine;
...
procedure TScintCustomStyler.ConsumeUntil(const s: TScintRawString; include : boolean);
begin
  while (FCurIndex <= FTextLen) and (copy(FText, FCurIndex, length(s)) <> s) do
    Inc(FCurIndex);
  if include then
    inc(FCurIndex, length(s));
end;
